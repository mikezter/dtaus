...
class BuchungTest < Test::Unit::TestCase
  ...
  def test_initialize
    buchung = Dtaus::Buchung.new(@konto_auftraggeber, @konto, 100.0, "Vielen Dank für Ihren Einfauf!")
    ...
    assert_equal "VIELEN DANK FUER IHREN EINFAUF!", buchung.text
  ...
  def test_initialize_incorrect_konto
    exception = assert_raise( Dtaus::DtausException ) do
      Dtaus::Buchung.new(123456789, @konto, 100.0, "Vielen Dank für Ihren Einfauf!")
    end
    assert_equal "Konto expected for Parameter 'auftraggeber', got Fixnum", exception.message

    exception = assert_raise( Dtaus::DtausException ) do
      Dtaus::Buchung.new(@konto, 123456789, 100.0, "Vielen Dank für Ihren Einfauf!")
    end
  ...
  def test_initialize_incorrect_betrag
    exception = assert_raise( Dtaus::DtausException ) do
      Dtaus::Buchung.new(@konto_auftraggeber, @konto, 100, "Vielen Dank für Ihren Einfauf!")
    end
    assert_equal "Betrag is a Fixnum, expected Float", exception.message

    exception = assert_raise( Dtaus::DtausException ) do
      Dtaus::Buchung.new(@konto_auftraggeber, @konto, 0.0, "Vielen Dank für Ihren Einfauf!")
    end
  ...
  def test_initialize_incorrect_erweiterungen
    exception = assert_raise( Dtaus::IncorrectSizeException ) do
      ...
      Dtaus::Buchung.new(konto_auftraggeber, konto, 100.0, "Vielen Dank für Ihren Einfauf!" * 5)
    end
  ...
  def test_to_dta
    ...
    assert_equal "0303C00000000123456781234567890000000000000005000 "+
                 ...
                 "                            AUFTRAGGEBER          "+
                 "     VIELEN DANK FUER IHREN EINF1  0401KUNDE      "+
                 "                03AUFTRAGGEBER                    "+
                 "      02VIELEN DANK FUER IHREN EINF02AUF!         "+
                 "                                                  "+
                 "                                  ", buchung.to_dta
  end
